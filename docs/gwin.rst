...
At the moment there is only a single choice ``--likelihood gaussian`` that is described in :py:class:`gwin.likelihood.GaussianLikelihood`.
...
These distributions are described in :py:mod:`gwin.distributions`.
...
You can reuse ``gwin.ini`` from the previous example to analyze a signal in real data::
