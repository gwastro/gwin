...
class GaussianLikelihood(_BaseLikelihoodEvaluator):
    ...
    >>> psds = {'H1': psd, 'L1': psd}
    >>> likelihood_eval = gwin.GaussianLikelihood(generator, signal, fmin, psds=psds)
    ...
    >>> from gwin import prior
    >>> uniform_prior = prior.Uniform(tc=(tsig-0.2,tsig+0.2))
    >>> prior_eval = prior.PriorEvaluator(['tc'], uniform_prior)
    >>> likelihood_eval = gwin.GaussianLikelihood(generator, signal, 20., psds=psds, prior=prior_eval)
